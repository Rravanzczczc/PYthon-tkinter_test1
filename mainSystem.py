...
def sort():
    ...
    if choice == '1':
        ...
        student_sort.sort(key=lambda s: int(s['C语言']) + int(s['Python']) + int(s['English']), reverse=asc_or_desc_bool)
    else:
        print('您输入的有误，请重新输入')
        sort()
...
# 项目打包，命令行里面使用  pip install PyInstaller  进行下载
# 使用指令完成项目打包：
# pyinstaller -F D:\JetBrains\pythonProject\Python_demo02_Pycharm\demo27_综合作业\mainSystem.py
# 找一下打包好的exe文件所在的目录，基本默认在C:\Users\Magic\dist   （Magic为个人电脑名称）
# 将student.txt 文件赋值过去放到同一层目录下面


# 1、自命题完成一个类似于学生管理系统的程序，功能数量>=8个
#       填写一下选题的名称
# 2、完成实验报告
# 3、完成之后准备一下项目演示
# 4、材料的提交：源码(.py文件)+报告(命名格式 姓名-学号)
#         格式：姓名-学号.zip
